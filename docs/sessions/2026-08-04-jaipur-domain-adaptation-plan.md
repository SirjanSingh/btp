...
`feat/init-project-setup` has its upstream set to `origin/master`, which is
wrong — a bare `git push` from this branch would target master. Pushed
explicitly to `origin feat/init-project-setup` this session. Worth fixing with
`git branch -u origin/feat/init-project-setup`.
